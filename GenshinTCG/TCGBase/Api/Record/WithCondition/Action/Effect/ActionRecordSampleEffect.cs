namespace TCGBase
{
    public record class ActionRecordSampleEffect : ActionRecordBaseWithTarget
    {
        ...
        public ActionRecordSampleEffect(int count = 1, int max = -1, List<string>? pool = null, TargetRecord? target = null, List<ConditionRecordBase>? when = null) : base(TriggerType.Effect, target, when)
        {
            Pool = pool ?? new();
        ...
        {
            var chars = Target.GetTargets(me, p, s, v, out var team).Where(pe => pe is Character c).ToList();
